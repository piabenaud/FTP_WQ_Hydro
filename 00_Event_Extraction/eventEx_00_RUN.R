...
if(!exists("foo", mode = "function")) source("eventEx_ReadFuncs.R")
## set up folders
...
# Read in data - these functions require user selection for the file. Supply the following args for your data:
# dateTime_col = column name for datetime (str)
# rain_col = column name for rainfall (str)
# response_col = column name for the response var that you are extracting events for e.g. flow (str)
# lubridate_time = the time format of your timestamp as a lubridate function e.g. ymd_hms or dmy_hms
# file_path = (optional) the file path for your input dataframe - if not supplied default will use file.choose()

# for reading RDS files:
...
colNames <- setNames(data.frame(matrix(ncol = 14, nrow = 0)),                   # Name columns of a new dataframe
                     c("userID",                                                #
                       "datetime",                                              # 
                       # "rainfall",                                              #
                       "sg",                                                    #
...
write.csv(stats, paste0("./post_eventEx_flow_wateryear_stats.csv"))
...
write.csv(stats, paste0("./post_eventEx_rain_wateryear_stats.csv"))
...
stats
write.csv(stats, paste0("./post_eventEx_rain_all_stats.csv"))
...
stats
write.csv(stats, paste0("./post_eventEx_flow_all_stats.csv"))
